...
class Style3Dependencies:
    def IsPYTouth():
        ...
        PagesManager.Main.Import("SeniorOS.style.home", "Page" + str(pagesNum))
        return
    ...
    def Page1():
        while not eval("[/GetButtonExpr('py')/]") or eval("[/GetButtonExpr('on')/]"):
            oled.fill(0)
            myUI = UI(oled)
            oled.DispChar(str('运行时间'), 5, 0, 1)
            oled.DispChar(str(time.time()) + "s", 5, 16, 1)
            oled.DispChar(str('今日已过'), 5, 32, 1)
