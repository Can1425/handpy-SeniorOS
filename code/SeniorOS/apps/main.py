    ...
    def main():
        ...
        while not button_a.is_pressed():
            ...
            if touchpad_t.is_pressed() and touchpad_h.is_pressed():
                options = eval('DayLight.Select.Style1(list{}, 28, True, "选择")'.format(str(settingsNum)),{'list0':['重连网络', '同步时间', '新建网络配置'],'list1':['日光模式', '动效开关','桌面风格','状态栏风格' '日光引擎信息'],'list2':['释放内存', '内存信息'],'DayLight':DayLight})
                if settingsNum == 0:
...
class App2:
    ...
    def main():
        path='/'
        while not button_a.is_pressed():
            App2.fileviewer(path)

    def IsFile(f):
        print(f)
        if os.stat(f)[0]<20000:return "目录"
        else:
            return App2.fileattribute(f)
    def fileattribute(f):
        last4=f[-4:]
        last3=f[-3:]
        if last3==".py":
    ...
    def fileviewer(initpath:str):
        ...
        if path[:2]=="//":
            path=path[1:]
        Pathpic=App2.pathpic
        Filepic=App2.filepic
        Runpic=App2.runpic
        Picpic=App2.picpic
