...
def home():
    ...
    while not (touchpad_t.is_pressed() and touchpad_h.is_pressed() or button_a.is_pressed() or button_b.is_pressed()):
        core.time_disposal()
        oled.fill(0)
        Fui.display_font(Flag_OS.fonts.quantum, (str(core.time_hour)), 33, 18, False)
        ui.display_font(Flag_OS.fonts.quantum, (str(core.time_min)), 67, 18, False)
...
def app():
    ...
    while not button_a.is_pressed():
        oled.fill(0)
        if str(core.get_file('./Flag_OS/data/Flag_sys_ui_light.fos', '\r\n')) == 'Open':
            oled.invert(1)
        else:
            oled.invert(0)
        ...
        if touchpad_t.is_pressed() and touchpad_h.is_pressed():
            ui.consani(home_movement_x, 6, 36, 36, 0, 0, 128, 64)
